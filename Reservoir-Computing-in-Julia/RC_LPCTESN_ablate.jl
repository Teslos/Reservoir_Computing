...
# ABLATION COPY of RC_LPCTESN.jl -- hyperparameters exposed as key=value ARGS so
# the closed-loop skill can be traced to a specific setting. Defaults are exactly
# the values in RC_LPCTESN.jl, so with no overrides this reproduces it.
argval(key, default) = begin
    i = findfirst(a -> startswith(a, key * "="), ARGS)
    i === nothing ? default : parse(Float64, ARGS[i][length(key)+2:end])
end

NR = quick ? 150 : Int(argval("NR", 300))          # reservoir size
dim_system = 3
spectral_radius = argval("rho", 1.1)
density = 0.1
sigma_in = argval("sigma_in", 0.5)  # input scaling (data is standardized)
leak = 1.0
speed = argval("speed", 10.0)   # reservoir time-scale factor: relaxation
                                # timescale is 1/(speed*leak) seconds
...
noise_level = argval("noise", 1e-3)  # state noise during fitting
...
println("RESULT speed=$speed rho=$spectral_radius sigma_in=$sigma_in noise=$noise_level beta=$ridge_beta method=LPCTESN$(quad ? "_quad" : "")$(partial ? "_partial" : "") seed=$SEED NR=$NR ",
