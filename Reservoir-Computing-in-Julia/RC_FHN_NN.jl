...
println("Topology: $topology, $(N) nodes, $(ne(g)) directed edges, ",
        "density $(round(Graphs.density(g), digits = 3)), coupling $coupling")
...
# Continue the teacher-forced input from the last training sample at t=0.
# Starting the ODE at t_test[1] would relabel r_end as the first future state.
